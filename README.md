...
|---|---|
| `main.py` | 训练入口与聚合后端选择 |
| `client/`、`server/` | 客户端训练和服务端聚合边界 |
| `runtime/` | device、paths、PEFT、broadcast、train step、loss 和 checkpoint |
| `utils/` | logger、timer、metrics、SVD、safetensors 和结果写入 |
| `models/`、`datasets/` | 本地模型与训练数据加载器 |
| `evaluation/` | MMLU、GSM8K 和公共评估逻辑 |
| `configs/` | 训练、收敛与评估配置 |
| `seclora/` | SecLoRA Python 适配、原生后端和 CUR 解码 |
| `logs/`、`output/`、`hf-cache/` | 运行时产物 |
