...
// factors: r2, k1 = g2^{-r2}, both tag terms, and all structural k2 bases.
// Online folds in the R real factors (slots 0..R-1).
iFE_FH_Core::KeyPrecomp iFE_FH_Core::keygen_precompute(
    const std::array<G2, 2>& tag) {
    Fr r2; r2.setByCSPRNG();
    return keygen_precompute(tag, r2);
}

iFE_FH_Core::KeyPrecomp iFE_FH_Core::keygen_precompute(
    const std::array<G2, 2>& tag, const Fr& r2) {
    KeyPrecomp pre;
    Fr neg_r2; Fr::neg(neg_r2, r2); G2::mul(pre.k1, g2_base, neg_r2);

    // k2[0] includes <s1_mask, tag>; online adds
    // g2^{sum s1[k] b_k}.
    Fr k2_0_scalar = r2 * s2[0];
    G2 g_part, t_part_0, t_part_1;
    G2::mul(g_part, g2_base, k2_0_scalar);
    G2::mul(t_part_0, tag[0], s1[2 * m_dim]);
    G2::mul(t_part_1, tag[1], s1[2 * m_dim + 1]);
    G2::add(pre.k2_0_base, g_part, t_part_0);
    pre.k2_0_base += t_part_1;

    pre.k2_base.resize(N);
    for (int i = 0; i < N; ++i) {
        Fr exp = r2 * s2[i + 1];
        G2 k2_i; G2::mul(k2_i, g2_base, exp);
        if (i == 2 * m_dim) k2_i += tag[0];
        if (i == 2 * m_dim + 1) k2_i += tag[1];
        pre.k2_base[i] = k2_i;
    }
    return pre;
}
...
// entries (the two mask coordinates); r1 is baked into every base term.
